        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <EngineBadge reason={path?.path?.last_recompute_reason} />
            </div>
 ...
 */
/**
 * L5 — surfaces which engine generated the active path.
 * Parses last_recompute_reason which carries strings like:
 *   "Initial path generated — 12 steps, 4 phases from Claude · Perplexity Sonar on goal create."
 *   "... from Claude · catalog (RAG) ..."
 *   "... from Claude · legacy (no candidates) ..."
 *   "... from demo template ..."
 */
function EngineBadge({ reason }) {
  if (!reason) return null;
  const lower = reason.toLowerCase();
  let label = null;
  let style = "bg-gray-50 border-gray-200 text-gray-600";
  let title = "";
  if (lower.includes("perplexity sonar")) {
    label = "🌐 Perplexity Sonar";
    style = "bg-purple-50 border-purple-200 text-purple-700";
    title = "Candidates from Perplexity Sonar (live web research). URLs are current; coverage is broad.";
  } else if (lower.includes("catalog (rag)") || lower.includes("rag-augmented")) {
    label = "📚 Catalog (RAG)";
    style = "bg-blue-50 border-blue-200 text-blue-700";
    title = "Candidates from the curated catalog via Pinecone semantic retrieval. Used as fallback when Perplexity is unavailable.";
  } else if (lower.includes("legacy")) {
    label = "⚠️ Legacy (Claude-invented)";
    style = "bg-amber-50 border-amber-200 text-amber-700";
    title = "Both Perplexity and catalog were unavailable. Steps were generated by Claude alone — URLs may not be verified.";
  } else if (lower.includes("demo template")) {
    label = "🧪 Stub template";
    style = "bg-gray-50 border-gray-200 text-gray-500";
    title = "Stub mode — ANTHROPIC_API_KEY isn't set, so a basic template was used.";
  } else {
    return null;
  }
  return (
    <span
      title={title}
      className={`inline-flex items-center gap-1 text-[9px] font-semibold rounded-full border px-1.5 py-0.5 ${style}`}
    >
      {label}
    </span>
  );
